"""Example of a simple environment where the agent has to stand up a humanoid robot. from Maniskill."""

from typing import Any, Dict, Union

import sapien
import torch
from mani_skill.agents.robots import UnitreeH1Simplified
...
class UnitreeH1StandEnv(HumanoidStandEnv):
    ...
    def _default_sim_config(self):
        return SimConfig(gpu_memory_cfg=GPUMemoryConfig(max_rigid_contact_count=2**22, max_rigid_patch_count=2**21))
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict) -> None:
        with torch.device(self.device):
            ...
            random_qpos = torch.randn(size=(b, self.agent.robot.dof[0]), dtype=torch.float) * 0.05
